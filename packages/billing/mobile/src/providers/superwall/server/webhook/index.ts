...
const getUserIdFromAttributes = (attrs: Record<string, unknown> | null | undefined) => {
  const candidates = [
    attrs?.referenceId,
...
export const webhookHandler = async (req: Request, callbacks?: WebhookCallbacks) => {
  ...
  const userIdFromAttributes = getUserIdFromAttributes(event.data.userAttributes ?? null);
  ...
  if (originalTransactionId) {
    ...
    await upsertSubscription({
      ...
      ...(event.data.periodType === "TRIAL"
        ? {
            trialStartsAt: event.data.purchasedAt ? dayjs(event.data.purchasedAt).toDate() : null,
            trialEndsAt: event.data.expirationAt ? dayjs(event.data.expirationAt).toDate() : null,
          }
        : {}),
    });
