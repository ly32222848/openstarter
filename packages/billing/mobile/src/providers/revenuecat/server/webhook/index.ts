...
import { upsertCustomer, upsertOrders, upsertSubscriptions } from "../../../../server/upserts";
import { HttpStatusCode, HttpException } from "../../../../server/http";
import { logger } from "@openstarter/shared/logger";

import { BillingProvider } from "../../../types";
import { env } from "../env";
import { toPaymentStatus, toSubscriptionStatus } from "../mappers/to-billing-status";
...
export const webhookHandler = async (req: Request, callbacks?: WebhookCallbacks) => {
  ...
  const subscriptions = Object.entries(subscriber.subscriptions ?? {})
    .map(([variantId, subscription]) => {
      const transactionId =
        subscription.original_transaction_id ?? subscription.store_transaction_id;
  ...
  const orders = Object.entries(subscriber.non_subscriptions ?? {}).flatMap(
    ([variantId, purchases]) =>
      purchases
        .map((purchase) => {
          const transactionId = purchase.original_transaction_id ?? purchase.store_transaction_id;
  ...
  const getOneTimePurchaseId = () => {
    ...
    const transactionId =
      matching?.original_transaction_id ?? matching?.store_transaction_id ?? event.transaction_id;
  ...
  if (subscriptions.length > 0) {
    ...
    logger.info(`✅ ${subscriptions.length} subscriptions upserted for customer ${customer.id}`);
  }

  if (orders.length > 0) {
    await upsertOrders(orders);
    logger.info(`✅ ${orders.length} orders upserted for customer ${customer.id}`);
  }
