...
/**
 * 公开分析配置（R25.1/R25.2 数据面）：仅含 web 端分析供应商标识与度量 ID（非敏感）。
 * 空字符串表示未配置该供应商——apps/web 据此决定是否注入对应脚本。
 *
 * 移动端分析供应商（GA mobile / OpenPanel）**不再**由本端点下发：App 是静态
 * 二进制，管理端改配置触达不到已发布的安装包，故改为随构建固化的
 * EXPO_PUBLIC_ANALYTICS_* env（见 apps/mobile/.env.example 与
 * packages/analytics-mobile/src/config.ts）。
 */
...
/**
 * 读取公开分析配置：从 Config 取 web 端分析供应商标识与度量 ID，去空白后返回；
 * 未配置项为空字符串。**绝不**下发其它（含敏感）配置项——仅白名单内的分析键。
 * （移动端供应商键已移除：移动端改为构建期 env 控制，见接口注释。）
 */
