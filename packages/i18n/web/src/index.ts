// @openstarter/i18n-web 包入口 —— 根导出翻译键类型与受支持语言集合。
// 供 packages/auth/src/types.ts 将认证错误码映射到翻译键（并归一其遗留的 @workspace/i18n 引用），
// 以及 apps/web / apps/extension / packages/email 等按同一 locale 集合与默认语言保持一致。
...
// project.inlang/settings.json 的 locales 与此保持一致；apps/web / apps/extension 的
// Paraglide 运行时据此编译。
